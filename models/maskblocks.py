...
from tools import focal_loss, dice_loss

from models.dsm import DSM
...
class Identity(nn.Module):
    ...
    def forward(self, img):
        ...
        Output:
        mask : mask of shape (1,height,width) indicating the pixels
        that are part of the seed expansion set
        height and width are the dimensions of the original image
        """
        w, h = img.size
...
class SAM(nn.Module):
    def __init__(self, size="b"):
        ...
        sam = sam_model_registry[f"vit_{size}"](
            checkpoint=f"/nasbrain/f21lin/{sizes[size]}")
        print("SAM loaded")
        sam.to("cuda")
...
class DeepSpectralMethods(nn.Module):
    ...
    def forward(self, img):
        """forward pass of the DeepSpectralMethods block"""

        img = self.transforms(img).unsqueeze(0).to("cuda")
        # returns a list of eigenvectors (arrays))
        eigenvectors = self.dsm(img)
        ...
        for i in range(len(eigenvectors)):
            mask = eigenvectors[i]
            mask = cv2.resize(
                mask, (224, 224), interpolation=cv2.INTER_NEAREST)
...
def combine_masks(masks_sam, masks_spectral, masks_lost,
                  norm: Union[float, bool], postprocess=True):
    """
    ...
    def postprocess_area(masks, threshold=0.5):
        ...
        in place operation"""
        for mask in masks:
            if mask["area"] > threshold * mask["segmentation"].shape[0] * \
                    mask["segmentation"].shape[1]:
    ...
    if postprocess is True:
        ...
        # intersection of the lost masks
        reference = np.prod([mask["segmentation"]
                            for mask in masks_lost], axis=0)
        reference = reference > 0
    ...
    for i in range(len(masks_lost)):
        for j in range(len(masks_sam)):
            losses_sam[j, i, 0] = focal_loss(
                masks_sam[j]["segmentation"], masks_lost[i]["segmentation"])
            losses_sam[j, i, 1] = dice_loss(
                masks_sam[j]["segmentation"], masks_lost[i]["segmentation"])
        for j in range(len(masks_spectral)):
            losses_spectral[j, i, 0] = focal_loss(
                masks_spectral[j]["segmentation"], masks_lost[i]["segmentation"])
            losses_spectral[j, i, 1] = dice_loss(
                masks_spectral[j]["segmentation"], masks_lost[i]["segmentation"])
    ...
    if norm is True:
        ...
        # focal / spectral
        losses_spectral[::, 0] /= np.sum(losses_spectral[:, :, 0])
        # dice / spectral
        losses_spectral[::, 1] /= np.sum(losses_spectral[:, :, 1])

    else:
        losses_sam[::, 0] *= norm  # focal / sam
        losses_spectral[::, 0] *= norm  # focal / spectral

    # sum the losses, shape (len(masks_sam), len(masks_lost))
    losses_sam = np.sum(losses_sam, axis=2)
    # sum the losses, shape (len(masks_spectral), len(masks_lost))
    losses_spectral = np.sum(losses_spectral, axis=2)

    # sum the losses, shape (len(masks_spectral),)
    losses_spectral = np.sum(losses_spectral, axis=1)
    # sum the losses, shape (len(masks_sam),)
    losses_sam = np.sum(losses_sam, axis=1)
...
def main(n, seed):
    ...
    dino_model = get_model(size="s", use_v2=False).to(
        "cuda")  # shared model for lost and dsm
    ...
    for _ in tqdm(range(n)):
        ...
        masks_sam, masks_spectral, masks_lost = combine_masks(
        ...
        nums_columns = 5
        _, axes = plt.subplots(len(masks) + 1, nums_columns, figsize=(10, 10))
        axes[0, 0].imshow(img)
        axes[0, 0].set_title("Original image")
        axes[0, 0].axis("off")

        for i, (mask, title) in enumerate(zip(masks, titles)):
            for j in range(min(nums_columns, len(mask))):
                axes[i + 1, j].imshow(mask[j]["segmentation"])
                if "loss" in mask[j].keys():
                    axes[i + 1,
                         j].set_title(f"{title}'s loss: {mask[j]['loss']:.1e}")
                else:
                    axes[i + 1,
                         j].set_title(f"{title}'s area: {mask[j]['area']:.1e}")

        for i in range(len(masks) + 1):
            for j in range(nums_columns):
                axes[i, j].axis("off")
