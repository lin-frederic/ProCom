...
class NCM(torch.nn.Module):
    ...
    def __init__(self, top_k=1):
        ...
        self.preprocess_ncm_layer = PreprocessFeatures()
        self.top_k = top_k
    ...
    def forward(
        ...
        support_features, query_features = self.preprocess_ncm_layer(
            support_features, query_features)
        acc = 0
        # calculate similarity between each query feature and each support
        # feature
        if use_cosine:
            similarity = F.cosine_similarity(
                query_features.unsqueeze(1),
                support_features.unsqueeze(0),
                dim=2)  # [n_query, n_shot]
        else:
            similarity = - \
                torch.cdist(query_features.unsqueeze(1), support_features.unsqueeze(0), p=2)
        ...
        visited = set()
        for _, (query_class, query_index) in enumerate(query_labels):
            # find where the other masks of the same image are
            if query_index not in visited:
                visited.add(query_index)
                indices = [j for j, (class_, index_) in enumerate(
                    query_labels) if index_ == query_index]
                query_similarity = similarity[indices, :]  # [n_masks, n_shot]
                _, n_shot = query_similarity.shape
                query_similarity = query_similarity.flatten()
                # the first n_shot elements are for the first mask, the next
                # n_shot for the second mask, etc
                _, sorted_indices = torch.sort(
                    query_similarity, descending=True)
                # get the top k indices
                top_k_indices = sorted_indices[:self.top_k]
                # get the top k masks, and the top k corresponding support
                # features
