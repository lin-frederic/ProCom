...
class FolderExplorer():
    """Explore the folder to get the path of the images"""

    def __init__(self, dataset_paths) -> None:
        self.dataset_paths = dataset_paths

    def __call__(self) -> Any:
        """returns a dict where the key is the dataset name and the value is a
        dict of list"""
        ...
        for dataset in self.dataset_paths:
            if dataset == "imagenet":
                ...
                for class_name in os.listdir(cfg.paths["imagenet"]):
                    dataset_dict["imagenet"][class_name] = [
                        os.path.join(
                            cfg.paths["imagenet"],
                            class_name,
                ...
                with open(os.path.join(cfg.paths["cub"], "train_test_split.txt"), 
                          "r", encoding="utf-8") as split_file:
                    train_test_split = split_file.readlines()
                    # filter train dataset with the image ids
                    test_dataset_image_ids = [
                        line.split()[0] for line in train_test_split if line.split()[1] == "1"]

                    test_dataset_images_ids_class = {}

                with open(os.path.join(cfg.paths["cub"], "images.txt"), "r", encoding="utf-8") as split_file:
                    ...
                    for line in images:
                        if line.split()[0] in test_dataset_image_ids:
                            ...
                            test_dataset_images_ids_class[class_name].append(
                                os.path.join(cfg.paths["cub"], "images", line))

                # {class: [image_id, ...]}
                dataset_dict["cub"] = test_dataset_images_ids_class

            elif dataset == "cifarfs":
                dataset_dict["cifarfs"] = {}
                # folder structure:
                # path_to_cifarfs/(meta_train/meta_test)/class_name/image_name
                metas = ["meta-test", "meta-val"]
                class_names = []
                for meta in metas:
                    for class_name in os.listdir(
                            os.path.join(cfg.paths["cifarfs"], meta)):
                        class_names.append(os.path.join(meta, class_name))
                for class_name in class_names:
                    image_names = os.listdir(os.path.join(
                        cfg.paths["cifarfs"], class_name))
                    dataset_dict["cifarfs"][class_name] = [
                        os.path.join(
                            cfg.paths["cifarfs"],
                            class_name,
                ...
                for class_name in os.listdir(
                    ...
                    dataset_dict["caltech"][class_name] = os.listdir(os.path.join(
                        cfg.paths["caltech"], "101_ObjectCategories", class_name))
                    dataset_dict["caltech"][class_name] = [img for img in dataset_dict["caltech"][class_name] if img.lower(
                    ).endswith(".jpg") or img.lower().endswith(".png") or img.lower().endswith(".jpeg")]
                    dataset_dict["caltech"][class_name] = [
                        os.path.join(
                            cfg.paths["caltech"],
                            "101_ObjectCategories",
                            class_name,
                            img) for img in dataset_dict["caltech"][class_name]]
            elif dataset == "food":
                dataset_dict["food"] = {}
                with open(os.path.join(cfg.paths["food"], "split_zhou_Food101.json"), "r", encoding="utf-8") as split_file:
                    ...
                    for item in split_dict["val"]:
                        ...
                        dataset_dict["food"][class_index].append(
                            os.path.join(cfg.paths["food"], "images", image_name))
            elif dataset == "flowers":
                dataset_dict["flowers"] = {}
                with open(os.path.join(cfg.paths["flowers"], "split_zhou_OxfordFlowers.json"), "r", encoding="utf-8") as split_file:
                    split_dict = json.load(split_file)
                    for item in split_dict["val"]:
                        image_name = item[0]
                        class_index = item[1]
                        ...
                        dataset_dict["flowers"][class_index].append(
                            os.path.join(cfg.paths["flowers"], "jpg", image_name))
            elif dataset == "pets":
                dataset_dict["pets"] = {}
                with open(os.path.join(cfg.paths["pets"], "split_zhou_OxfordPets.json"), "r", encoding="utf-8") as split_file:
                    split_dict = json.load(split_file)
                    for item in split_dict["val"]:
                        image_name = item[0]
                        class_index = item[1]
                        ...
                        dataset_dict["pets"][class_index].append(
                            os.path.join(cfg.paths["pets"], "images", image_name))
...
class EpisodicSampler():
    # for each dataset, sample n_ways classes, then sample n_shot images for
    # each class, then sample n_query images for each class
    def __init__(
            self,
            paths,
            n_shot=1,
            n_ways={
                dataset: 5 for dataset in cfg.paths.keys()},
            n_query=16) -> None:
        self.n_shot = n_shot
    ...
    def __call__(self, seed_classes=42, seed_images=42) -> Any:
        ...
        for dataset in self.paths:
            ...
            selected_classes = rd.sample(
                list(
                    self.paths[dataset].keys()),
                self.n_ways[dataset])
            ...
            for classe in selected_classes:
                ...
                if seed_images is not None:
                    rd.seed(seed_images)
                shuffle = rd.sample(self.paths[dataset][classe], min(
                    self.n_shot + self.n_query, len(self.paths[dataset][classe])))
...
class DatasetBuilder():
    ...
    def __call__(self, seed_classes=None, seed_images=None) -> Any:
        ...
        for dataset_name, list_classes in episode.items():
            support_images = [
                image_path for classe in list_classes for image_path in list_classes[classe]["support"]]
            support_labels = [
                classe for classe in list_classes for image_path in list_classes[classe]["support"]]
            query_images = [
                image_path for classe in list_classes for image_path in list_classes[classe]["query"]]
            query_labels = [
                classe for classe in list_classes for image_path in list_classes[classe]["query"]]
    ...
    for dataset_name, list_classes in paths.items():
        ...
        if len(list_classes) > 0:
            ...
            for classe in list_classes:
                if not (all(img.lower().endswith(".jpg") or img.lower().endswith(
                        ".png") or img.lower().endswith(".jpeg") for img in list_classes[classe])):
...
def main_bis():
    """Test the EpisodicSampler class"""
    folder_explorer = FolderExplorer(cfg.paths)
    ...
    episodic_sampler = EpisodicSampler(
        paths,
        n_ways={
            dataset: 2 for dataset in cfg.paths.keys()},
        n_shot=1,
        n_query=2)
...
def main_ter():
    ...
    dataset_builder = DatasetBuilder(cfg.paths)
