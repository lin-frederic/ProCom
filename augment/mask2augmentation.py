"""Generate the embedding of the masks"""
...
class Mask2embedings():
    """Class to generate the embedding of the masks"""
    def __init__(self,
                 # {path_to_image : [(mask 1 of the image, crop_bbox correponding to the mask), (mask 2, bbox) ...]}
                 masks: Dict,
                 path_to_save: str,
                 embedding: List[str],
                 ):
        ...
        # {img_path : [masks list with the best mask of the image, with their relative corp link to each mask]}
        self.masks = masks
    ...
    def generate_embedding(self) -> Dict:
        ...
        for path in self.img_paths:
            ...
            for embedding in self.emdeddings:
                # compute the choosen embedding (in the emb list) for each
                # masks

                # list with all the embedded images for each mask for the
                # choosen embedding
                embedded_masks = []
                if embedding == "gaussian noise":
                    for mask in range(len(masks)):
                        emb_img = emb.gaussian_noise(
                            image, mask[0])  # return an image
                        if len(mask) > 1:
                            # mask[1] is the bbox
                ...
                # add the embedded image to the names of the embeding in the
                # global dict
    ...
    def save_masks(self):
        ...
        with open(self.path_to_save, "w", encoding="utf-8") as json_file:
            json.dump(img2masks, json_file)
        print(
            f"Mask and images generated saved at {self.path_to_save}")
