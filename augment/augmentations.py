"""This module contains functions to augment images given a mask"""

import numpy as np
import scipy.ndimage as sc
import cv2
from PIL import Image
# black background


def remove_bg(image, mask):
    """Remove the background of an image given a mask"""
    image_masked = image.copy()
    # Set the BGR color to [0, 0, 0] for black
    image_masked[mask == False] = [0, 0, 0]
    #image_masked = cv2.cvtColor(image_masked, cv2.COLOR_BGR2RGB)
    return image_masked


def crop(image, bbox, dezoom=0.3):
    """Crop an image given a bounding box"""
    # z is an additional margin
    if isinstance(image, Image.Image):
        image = np.array(image)
    h, w, _ = image.shape
    h_crop = bbox[3]
    w_crop = bbox[2]
    if h_crop / h < dezoom:  # if the crop is too small, we dezoom
        h_crop = int(h * dezoom)
    elif w_crop / w < dezoom:
        w_crop = int(w * dezoom)
    image = image[int(bbox[1]):max(round(bbox[1] + h_crop), h),
                  int(bbox[0]):max(round(bbox[0] + w_crop), w), :]
    return Image.fromarray(image)


def crop_mask(image, mask):
    """Crop an image given a mask"""
    #mask is (H,W)
    #image is (H,W,3)

    # if the mask is empty, we return the image
    if len(np.where(mask)[0]) == 0 or len(np.where(mask)[1]) == 0:
        return image

    x_min = np.min(np.where(mask)[1])
    y_min = np.min(np.where(mask)[0])
    x_max = np.max(np.where(mask)[1])
    y_max = np.max(np.where(mask)[0])
    bbox = [x_min, y_min, x_max - x_min, y_max - y_min]
    return crop(image, bbox)


def gaussian_noise(image, mask, sigma=1.5):
    """Add Gaussian noise to an image given a mask"""
    # Load your image and mask
    original_image = image.copy()  # Replace with your image data

    # Apply Gaussian noise to the entire image
    noisy_image = sc.gaussian_filter(original_image, sigma)

    # Retain original pixel values where the mask is True
    noisy_image[mask] = original_image[mask]
    return noisy_image


def highlighted_image(image, mask):
    """Highlight important elements of an image given a mask"""
    # Highlight important elements with a red border
    image_with_highlight = image.copy()
    # Set color (BGR) for important elements
    image_with_highlight[mask] = [0, 0, 255]
    return image_with_highlight


def gaussian_blur(image, mask, sigma=1.5):
    """Blur an image given a mask"""
    # Load your image and mask
    original_image = image.copy()  # Replace with your image data

    # Apply Gaussian noise to the entire image
    noisy_image = cv2.GaussianBlur(
        original_image,
        (0,
         0),
        sigmaX=sigma,
        sigmaY=sigma,
        borderType=cv2.BORDER_REPLICATE)

    # Retain original pixel values where the mask is True
    noisy_image[mask] = original_image[mask]
    return noisy_image


def highlighted_contour(image, mask):
    """Highlight the contour of an image given a mask"""
    # Create a copy of the image
    highlighted_img = image.copy()

    # Find the boundary between True and False elements in the mask
    contours, _ = cv2.findContours(mask.astype(
        np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Draw a red outline around the boundary
    cv2.drawContours(highlighted_img, contours, -
                     1, (0, 0, 255), thickness=2)  # BGR color (red)
    return highlighted_img
